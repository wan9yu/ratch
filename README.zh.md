...
- `no-forbidden-literal` — 你传入的字面量；空 `patterns=` 为 VACUOUS；`exclude_paths=` 跳过内容和文件名
...
- `bdd-test-conventions` — 测试名读成一句话；`prefix=` / `blank_blocks=` / `forbid_comment_labels=`
...
- `injected-clock` — 有 Clock 类型时禁止绑定后的 stdlib time/datetime 调用；`extra_time_attrs=` / `extra_dt_attrs=`
- `confined-import` — 列出的 import 只许在 `allow_paths=`；空 `names=` 为 VACUOUS
